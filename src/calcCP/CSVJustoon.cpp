...
bool CSVJustoon::ReadFile(QString filepath)
{
	m_CSVData = QtCSV::Reader::readToList(filepath);
	if (m_CSVData.at(CSV_START_ROW).size() == HEADER_JUSTOON_MAX) {
		return true;
	}
	return false;
}
...
void CSVJustoon::SetItem()
{
	m_ProgressWidget->InitProgress("Calculate", CSV_START_ROW, m_CSVData.size(), CalcCPDlg::CP_JUSTOON, CalcCPDlg::CP_MAX);
