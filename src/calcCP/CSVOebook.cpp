...
{
	m_ProgressWidget->InitProgress("Extract", 0, m_CSVModel->rowCount(), CalcCPDlg::CP_OEBOOK, CalcCPDlg::CP_MAX);
...
{
	m_ProgressWidget->InitProgress("Calculate", CSV_START_ROW, m_CSVData.size(), CalcCPDlg::CP_OEBOOK, CalcCPDlg::CP_MAX);
	...
	for (int i = CSV_START_ROW; i < m_CSVData.size() - 1; i++) {
		...
		// oebook
		if (!m_CSVData.at(i).value(HEADER_OEBOOK_COMPANY).compare(QString::fromLocal8Bit("¿ÀÀÌºÏ"))) {
		...
		// oebook romance
		else if (!m_CSVData.at(i).value(HEADER_OEBOOK_COMPANY).compare(QString::fromLocal8Bit("¿ÀÀÌºÏ(·Î¸Á)"))) {
		...
		// oebook murim
		else if (!m_CSVData.at(i).value(HEADER_OEBOOK_COMPANY).compare(QString::fromLocal8Bit("¿ÀÀÌºÏ(¹«¸²)"))) {
		...
		// oebook comic
		else if (!m_CSVData.at(i).value(HEADER_OEBOOK_COMPANY).compare(QString::fromLocal8Bit("¿ÀÀÌºÏ(ÄÚ¹ÍÇÃ·¯½º)"))) {
