...
void CSVTotalCP::InitCPData()
{
	...
	m_CSVModel = new QStandardItemModel(ROW_AMOUNT_MAX, CalcCPDlg::CP_TYPE::CP_MAX + 4);
...
void CSVTotalCP::SetItem(CalcCPDlg* cpData)
{
	...
	if (cpData->GetKyobo()) {
		m_CSVModel->setHeaderData(CalcCPDlg::CP_KYOBO + 1, Qt::Horizontal, QString::fromLocal8Bit(CPNAME_KYOBO.toStdString().c_str()));
	...
	if (cpData->GetNaver()) {
		m_CSVModel->setHeaderData(CalcCPDlg::CP_NAVER + 1, Qt::Horizontal, QString::fromLocal8Bit(CPNAME_NAVER.toStdString().c_str()));
	...
	if (cpData->GetRidi()) {
		m_CSVModel->setHeaderData(CalcCPDlg::CP_RIDI + 1, Qt::Horizontal, QString::fromLocal8Bit(CPNAME_RIDI.toStdString().c_str()));
	...
	if (cpData->GetMunpia()) {
		m_CSVModel->setHeaderData(CalcCPDlg::CP_MUNPIA + 1, Qt::Horizontal, QString::fromLocal8Bit(CPNAME_MUNPIA.toStdString().c_str()));
	...
	if (cpData->GetMrblue()) {
		m_CSVModel->setHeaderData(CalcCPDlg::CP_MRBLUE + 1, Qt::Horizontal, QString::fromLocal8Bit(CPNAME_MRBLUE.toStdString().c_str()));
	...
	if (cpData->GetBarobook()) {
		m_CSVModel->setHeaderData(CalcCPDlg::CP_BAROBOOK + 1, Qt::Horizontal, QString::fromLocal8Bit(CPNAME_BAROBOOK.toStdString().c_str()));
	...
	if (cpData->GetBookcube()) {
		m_CSVModel->setHeaderData(CalcCPDlg::CP_BOOKCUBE + 1, Qt::Horizontal, QString::fromLocal8Bit(CPNAME_BOOKCUBE.toStdString().c_str()));
	...
	if (cpData->GetEpyrus()) {
		m_CSVModel->setHeaderData(CalcCPDlg::CP_EPYRUS + 1, Qt::Horizontal, QString::fromLocal8Bit(CPNAME_EPYRUS.toStdString().c_str()));
	...
	if (cpData->GetOebook()) {
		m_CSVModel->setHeaderData(CalcCPDlg::CP_OEBOOK + 1, Qt::Horizontal, QString::fromLocal8Bit(CPNAME_OEBOOK.toStdString().c_str()));
	...
	if (cpData->GetOnestore()) {
		m_CSVModel->setHeaderData(CalcCPDlg::CP_ONESTORE + 1, Qt::Horizontal, QString::fromLocal8Bit(CPNAME_ONESTORE.toStdString().c_str()));
	...
	if (cpData->GetKakao()) {
		m_CSVModel->setHeaderData(CalcCPDlg::CP_KAKAO + 1, Qt::Horizontal, QString::fromLocal8Bit(CPNAME_KAKAO.toStdString().c_str()));
	...
	if (cpData->GetComico()) {
		m_CSVModel->setHeaderData(CalcCPDlg::CP_COMICO + 1, Qt::Horizontal, QString::fromLocal8Bit(CPNAME_COMICO.toStdString().c_str()));
	...
	if (cpData->GetTocsoda()) {
		m_CSVModel->setHeaderData(CalcCPDlg::CP_TOCSODA + 1, Qt::Horizontal, QString::fromLocal8Bit(CPNAME_TOCSODA.toStdString().c_str()));
	...
	if (cpData->GetKepub()) {
		m_CSVModel->setHeaderData(CalcCPDlg::CP_KEPUB + 1, Qt::Horizontal, QString::fromLocal8Bit(CPNAME_KEPUB.toStdString().c_str()));
