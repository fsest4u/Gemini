...
{
	InitProgress(ROW_AMOUNT, m_BookList.count(), CalcCPDlg::CP_MAX);
...
void CSVTotalBook::InitProgress(int min, int max, int index)
{
	if (m_Progress) {
		delete m_Progress;
		m_Progress = NULL;
	...
	QString label = QString("Book %1 Data ... ").arg(CP_NAME.at(index))
